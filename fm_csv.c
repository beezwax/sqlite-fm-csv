/*
** Forked 2024-06-14 to optionally use carriage returns instead of newlines as EOL markers.
**
** 2016-05-28
**
...
**
**    .load ./csv_fm
**    CREATE VIRTUAL TABLE temp.csv USING csv(crlines,filename=FILENAME);
**    SELECT * FROM csv;
**
...
struct CsvReader {
  ...
  char linedelim;		// PATCHED: added for adjustable line delimiter
};
...
static void csv_reader_init(CsvReader *p){
  ...
  syslog (LOG_NOTICE, "csv_reader_init: starting, linedelim=%c", p->linedelim);
}
...
static int csv_reader_open(
  ...
  const char *zData           /*  ... or use this data */
	...
	syslog (LOG_NOTICE, "csv_reader_open: starting, linedelim=%c", p->linedelim);
...
static char *csv_read_one_field(CsvReader *p){
  int c;
	...
	syslog (LOG_NOTICE, "csv_read_one_field: starting, linedelim=%c", p->linedelim);
  ...
  if( c=='"' ){
    ...
    while( 1 ){
      c = csv_getc(p);
      if( c<='"' || pc=='"' ){
        if( c==p->linedelim) p->nLine++;	// PATCHED: use global
        if( c=='"' ){
          if( pc=='"' ){
            pc = 0;
            continue;
        ...
        if( (c==',' && pc=='"')
         || (c==p->linedelim && pc=='"')				// PATCHED: use global
         || (c=='\n' && pc=='\r' && ppc=='"')
         || (c==EOF && pc=='"')
        ){
			syslog (LOG_NOTICE, "csv_read_one_field: hit delim, c='%c', pc='%c", c, pc);
    ...
    ** UTF-8 BOM  (0xEF BB BF) then skip the BOM */
    ...
    while( c>',' || (c!=EOF && c!=',' && c!=p->linedelim) ){	// PATCHED: use global
      if( csv_append(p, (char)c) ) return 0;
      c = csv_getc(p);
    }
	  syslog (LOG_NOTICE, "csv_read_one_field: hit field delim, c='%c'", c);

    if( c==p->linedelim ){		// PATCHED: use global
      p->nLine++;
      if( p->n>0 && p->z[p->n-1]=='\r' ) p->n--;		// how is \r being used here?
    }
    p->cTerm = (char)c;
  }
...
**    crlines=YES|NO             "yes" if should use CR's instead of newlines for line endings. Defaults to "no".
**
...
static int csvtabConnect(
  ...
  int bCrLines = -1;         /* crlines= flags.  PATCHED: added */
...
# define CSV_SCHEMA   (azPValue[2])
  ...
  syslog (LOG_NOTICE, "csvtabConnect: starting");
  ...
  for(i=3; i<argc; i++){
	...
	// PATCHED: added handler for crlines option
    if( csv_boolean_parameter("crlines",7,z,&b) ){
      if( bCrLines >= 0 ){
        csv_errmsg(&sRdr, "more than one 'crlines' parameter");
        goto csvtab_connect_error;
      }
      bCrLines = b;
    }else
...
static void csvtabCursorRowReset(CsvCursor *pCur){
  ...
  int i;
	...
	syslog (LOG_NOTICE, "csvtabCursorRowReset: starting");
...
static int csvtabClose(sqlite3_vtab_cursor *cur){
	...
	syslog (LOG_NOTICE, "csvtabClose: starting");
...
static int csvtabOpen(sqlite3_vtab *p, sqlite3_vtab_cursor **ppCursor){
  ...
  size_t nByte;
	...
	syslog (LOG_NOTICE, "csvtabOpen: starting");
  ...
  *ppCursor = &pCur->base;
  ...
  pCur->rdr.linedelim = '\n';	// PATCHED: Do this after rdr has been init'd by csv_reader_open
  return SQLITE_OK;
}

/*
** Constructor for a new CsvTable cursor object
** PATCHED: USING CR LINE DELIMITER.
*/
static int csvtabOpenFM(sqlite3_vtab *p, sqlite3_vtab_cursor **ppCursor){
  CsvTable *pTab = (CsvTable*)p;
  CsvCursor *pCur;
  size_t nByte;
  ...
  *ppCursor = &pCur->base;
  ...
  pCur->rdr.linedelim = '\r';	// Do this after rdr has been init'd by csv_reader_open
